...
// Função para desenhar um caractere no display SSD1306 (8x8 pixels)
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
  ...
  for (uint8_t i = 0; i < 8; ++i) {
    ...
    for (uint8_t j = 0; j < 8; ++j) {
      if (c >= 'a' && c <= 'z') {
        // Letras minúsculas: inverter linhas e colunas (rotacionar o caractere)
        ssd1306_pixel(ssd, x + j, y + i, (line & (1 << (7 - j))) ? 1 : 0);
      } else {
        // Letras maiúsculas e dígitos mantêm o padrão original
